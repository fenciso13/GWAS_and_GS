...
> plot(y~z)
```
![ScreenShot](https://github.com/fenciso13/GWAS_and_GS/blob/master/pdf/q_plot1.pdf)
```{r}
tmp=(quantile(y,probs = c(0.2,0.4,0.6,0.8)))
> z=ifelse(y<=tmp[1],1, ifelse(y<=tmp[2],2, ifelse(y<=tmp[3],3, ifelse(y<=tmp[4],4,5))))
> plot(y~z)
```
![ScreenShot](https://github.com/fenciso13/GWAS_and_GS/blob/master/pdf/q_plot2.pdf)
