...
lineNm=c() # add the names of the lines to remove here ie."stub-71",  "stub-640", "stub-10"
```
...
## Miscelaneus: Joining two files by commun snps
rm(list=ls())
fileIn<-'X1.RData'
load(fileIn)
fileIn2<-'X2.RData'
...
comun = intersect(colnames(X1),colnames(X2))
X1 = X1[,comun]
X2 = X2[,comun]
...
if(sum(colnames(X1)!=colnames(X2))>0) stop("check for problems")

X = rbind(X1,X2)

```
[Next_module](https://github.com/fenciso13/GWAS_and_GS/blob/master/modules/2.%20Genotype%20analysis.md)
